...
include_once "../../domain/tb_pedidos.php";
...
if (!empty($data->idCliente) && !empty($data->precoNota) && !empty($data->notaFis)){ //"!" - é uma negação 

    $tb_pedidos->idCliente = $data->idCliente;
    $tb_pedidos->precoNota = $data->precoNota;
    $tb_pedidos->notaFis = $data->notaFis;

    if($tb_pedidos->atualizar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"pedidos atualizado com sucesso!"));
