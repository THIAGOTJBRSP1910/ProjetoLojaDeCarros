...
if (!empty($data->notaFis)){ //"!" - é uma negação 

    $tb_carrinho->notaFis=$data->notaFis;
    
    if($tb_carrinho->apagar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Nota apagado com sucesso"));
    }
    else{
        header("HTTP/1.0 400");
    ...
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Voce deve passar o Id da nota"));
}
?> 