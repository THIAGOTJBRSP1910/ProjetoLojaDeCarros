...
include_once "../../domain/tb_carrinho.php";
...
if (!empty($data->notaFis) && !empty($data->codigoVeiculo) && !empty($data->quant) && !empty($data->prcUnitario)){ //"!" - é uma negação 

    $tb_carrinho->notaFis=$data->notaFis;
    $tb_carrinho->codigoVeiculo = $data->codigoVeiculo;
    $tb_carrinho->quant = $data->quant;
    $tb_carrinho->prcUnitario = $data->prcUnitario;
    

if($tb_carrinho->cadastrar()){
    header("HTTP/1.0 201");
    echo json_encode(array("mensagem"=>"Produto cadastrado no carrinho com sucesso"));
}
