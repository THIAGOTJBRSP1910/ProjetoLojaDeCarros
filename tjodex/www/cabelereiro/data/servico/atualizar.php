...
//Instância da classe servico
$servico = new Servico($db);

/*
Vamos preparar o php para receber os dados que estão sendo enviados pelo servico
...
if(!empty($data->tiposervico) && !empty($data->idpreco) && !empty($data->idservico)){
    $servico->tiposervico = $data->tiposervico;
    $servico->idpreco = $data->idpreco;
    $servico->idservico=$data->idservico;

    if($servico->atualizar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Serviço atualizado com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
